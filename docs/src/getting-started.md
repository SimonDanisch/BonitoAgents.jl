...
The desktop entry point boots the dashboard server *and* a worker for the
...
All state — projects, chat history, the machine's worker identity — persists
across restarts under the platform data directory
(`~/.local/share/BonitoAgents` on Linux, `~/Library/Application
Support/BonitoAgents` on macOS, `%LOCALAPPDATA%\BonitoAgents` on Windows).
...
Run the server where it is always reachable — a home server, a VPS:
...
Then, on **each machine that should run agents**, paste the one-liner from
...
environment, registers the machine with your server under a stable identity,
and (on Linux) sets up a systemd user service so the worker survives reboots.
The machine appears in the dashboard seconds later. Re-run the same one-liner
any time to update — it always installs the code revision the server itself
is running.
...
- **Discover** — the worker scans for existing Claude Code sessions on that
  machine; any folder you've used `claude` in shows up and can be imported
  *including its conversation history*.
