...
export TestServer, dev_server, add_worker!,
       text, thought, edit, bash, todo, delay, tool, tool_update, config_update,
...
# Agent-driven mid-turn session config change. `options` is the COMPLETE
# `configOptions` list (the chat REPLACES its config view from it) — emitted as
# a `config_option_update` session notification, the way claude-agent-acp tells
# the client the model/mode/effort changed. The mock needs config enabled
# (`mock_env = Dict("BT_MOCK_ACP_CONFIG" => "1")` on `dev_server`).
config_update(options::AbstractVector) = Dict("type" => "config_update", "options" => options)
...
function dev_server(; agent::Function = (_msg -> end_turn()),
                      ...
                      mock_env::AbstractDict = Dict{String,String}(),
                      kwargs...)
    ensure_display!()
    ...
    # Per-suite mock toggles (e.g. BT_MOCK_ACP_CONFIG=1 to advertise the
    # session-config blocks) reach the worker's MockACP via the same ENV path.
    for (k, v) in mock_env
        agent_env[String(k)] = String(v)
    end
