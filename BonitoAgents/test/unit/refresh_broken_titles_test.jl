...
# message (gated by `p.title[] == ""`). When the wrapper-stripping
...
function seed_project!(state::BT.ServerState, worker_id::String;
...
@testset "refresh_broken_titles! repairs leaked-wrapper titles" begin
    ...
    @testset "leaked ide_selection (no closer) → back to the folder name if the prompt is unparseable too" begin
        ...
        # original prompt. The sweep MUST put the default back so the next
    ...
    @testset "default title is left alone (no chat history yet)" begin
        state = fresh_state()
        wid   = "wA"
    ...
    @testset "only touches projects on the scanned worker" begin
        state = fresh_state()
        seed_project!(state, "wA"; pid="pA", name="A",
                     title="<command-args/> a",
                     prompt="<command-args/>\nprompt A")
        pB, _ = seed_project!(state, "wB"; pid="pB", name="B",
