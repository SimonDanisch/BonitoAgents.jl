...
# When set (BT_MOCK_ACP_CONFIG=1), session/new + session/load advertise the
# permission-mode / model / effort config blocks the real claude-agent-acp
# (0.42.0) sends, so tests can exercise the chat-header session-config controls.
# Default off so other suites' headers stay config-free.
CONFIG::Bool            = false
...
function _configure!()
    ...
    global CONFIG          = String(get(ENV, "BT_MOCK_ACP_CONFIG", "")) == "1"
    return nothing
end

# The session-config blocks a config-capable agent reports in its session/new
# (and session/load) result: typed `configOptions` plus the `models`/`modes`
# blocks the chat falls back to. Field shapes mirror a live claude acp.jsonl.
# Tests drive mid-session changes with the dispatcher `config_update` event.
config_blocks() = Dict{String,Any}(
    "configOptions" => [
        Dict("id"=>"mode", "name"=>"Mode", "category"=>"mode",
        ...
        Dict("id"=>"model", "name"=>"Model", "category"=>"model",
             "type"=>"select", "currentValue"=>"default", "options"=>[
        ...
        Dict("id"=>"effort", "name"=>"Effort", "category"=>"thought_level",
             "type"=>"select", "currentValue"=>"default", "options"=>[
                Dict("value"=>"default", "name"=>"Default"),
    ...
    "models" => Dict("currentModelId"=>"default", "availableModels"=>[
        ...
        Dict("modelId"=>"sonnet", "name"=>"Sonnet")]),
    "modes" => Dict("currentModeId"=>"default", "availableModes"=>[
        Dict("id"=>"default", "name"=>"Default"),
        Dict("id"=>"plan", "name"=>"Plan"),
        Dict("id"=>"bypassPermissions", "name"=>"Bypass Permissions")]),
)
...
function run_dispatcher_prompt(prompt_id)
    ...
    while !eof(sock)
        ...
        elseif et == "config_update"
            # Agent-driven mid-turn session config change: emit a
            # `config_option_update` carrying the COMPLETE configOptions list.
            # The chat patches `session_meta` + re-renders the header pills
            # WITHOUT splitting the open agent bubble.
            upd("config_option_update",
                Dict("configOptions" => get(ev, "options", Any[])))
        elseif et == "thought"
...
function dispatch_loop()
    while !eof(stdin)
        ...
        elseif method == "session/new" && id !== nothing
            resp(id, merge(Dict{String,Any}("sessionId" => SESSION),
                           CONFIG ? config_blocks() : Dict{String,Any}()))
        elseif method == "session/load" && id !== nothing
            resp(id, merge(Dict{String,Any}("sessionId" => SESSION),
                           CONFIG ? config_blocks() : Dict{String,Any}()))
        elseif method == "session/set_config_option" && id !== nothing
            # Ack the chat's config assertion (bring-up asserts mode/effort; the
            # user picking a header pill sends one too). The real agent replies
            # with the complete updated configOptions; an empty object is the
            # documented minimal ack and the chat falls back to its optimistic
            # patch (see `adopt_config_options!`). Always answering keeps the
            # request from dangling (the e2e leak audit asserts no pending RPCs).
            resp(id, Dict{String,Any}())
