...
function seed_discovered!(server, wid)
    ...
    # No automatic scan can overwrite this: test servers don't scan a worker on
    # connect (`TK.dev_server(; scan_on_connect = false)`). One that did would
    # render the real sessions of the box running the suite and race the seed.
    base   = mktempdir()
